...
class TankDesignUploadForm extends HTMLElement {
	...
	connectedCallback() {
		...
		var wc = new WebComponentLoader(this, "TankDesignUploadForm/TankDesignUploadForm");
	...
	execute(component){
		...
		submitButton.onclick = component.submitTankDesign(component);
	...
	submitTankDesign(component){
		return function(){
			var fieldsPopulated = component.verifyUploadInputValuesPopulated(component);
			if(fieldsPopulated){
				component.showModal(component, 'LOADING', 'Upload in progress...');
				try{
					$.ajax({
				        url: 'https://ultimate-summer-party.herokuapp.com/Tank/Design/Entry',
				        type: 'POST',
				        data: new FormData(component.shadowRoot.querySelector('#tankDesignEntryForm')),
				        cache: false,
				        contentType: false,
				        processData: false,
				        xhr: function() {
				            return component.createCustomFileUploadXHRRequest()
				        },
				        success: (data) => {
							component.hideModal(component);
	...
	verifyUploadInputValuesPopulated(component){
		...
		if(!fieldsPopulated){
			component.showModal(component, "ERROR", "You must populate all fields to submit a design");
		}
...
export default TankDesignUploadForm;